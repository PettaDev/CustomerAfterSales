...
test("hardware warranty triage is optional and customer replies require an open support request", async () => {
  ...
  process.env.STAFF_PASSWORD_HASH = encodeSecret("test-password-123");
  process.env.STAFF_ID = "tester";
  process.env.STAFF_NAME = "Test TFAE";
  ...
  try {
    ...
    const prematureReply = await fetch(base + "/api/cases/" + body.case.id + "/customer-replies", {
      method: "POST",
      headers: {
    ...
    assert.equal(prematureReply.status, 409);

    const login = await fetch(base + "/api/auth/login", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: "tester@example.com", password: "test-password-123" }),
    });
    assert.equal(login.status, 200);
    const cookie = login.headers.get("set-cookie").split(";")[0];

    const requestMore = await fetch(base + "/api/cases/" + body.case.id, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Cookie: cookie },
      body: JSON.stringify({ status: "awaiting_customer", note: "Please confirm the requested validation." }),
    });
    assert.equal(requestMore.status, 200);
