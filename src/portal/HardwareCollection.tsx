...
export function hardwareCollectionCopy(language: string, chooseFiles: string, remove: string): HardwareCollectionCopy {
  if (language.startsWith("pt")) return {
    ...
    evidenceText: "Envie imagens nítidas do aparelho e, se possível, um vídeo reproduzindo o problema. Se o aparelho puder estar na garantia e você tiver nota fiscal ou comprovante de compra, inclua também uma foto legível. Não envie logs técnicos para casos de hardware.",
  ...
  if (language.startsWith("es")) return {
    ...
    evidenceText: "Sube imágenes claras del dispositivo y, si es posible, un video reproduciendo la falla. Si el dispositivo puede estar en garantía y tienes factura o comprobante de compra, incluye también una foto legible. No envíes logs técnicos para casos de hardware.",
  ...
  if (language.startsWith("zh")) return {
    ...
    evidenceTitle: "照片和视频",
    evidenceText: "请上传清晰的设备照片，并尽可能上传复现问题的视频。如果设备可能仍在保修期内且您有发票或购买凭证，请同时上传一张清晰照片。硬件问题无需上传技术日志。",
  ...
  return {
    ...
    evidenceText: "Upload clear photos of the device and, when possible, a video reproducing the issue. If the device may still be under warranty and you have a receipt or proof of purchase, include a clear photo as well. Do not upload technical logs for hardware cases.",
